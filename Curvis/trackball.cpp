...
void quat_to_mat(Quaternion q, mat4 mat)
{
	float s;
	float xs, ys, zs;
	float wx, wy, wz;
...
void mat_to_quat(mat4 mat, Quaternion q)
{
	int i, j, k;
	float tr, s;
...
static void mcopy(const mat4 m1, mat4 m2)
{
	int i, j;
	for (i = 0; i < 4; i++)
		for (j = 0; j < 4; j++)
...
static void minvert(const mat4 mat, mat4 result)
{
	int i, j, k;
	double temp;
	double m[8][4];
